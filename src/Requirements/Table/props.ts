...
interface IAction {
  id: string;
  actionName: string;
  disabled?: (entry: IActions) => boolean;
  content: (entry: IActions, onActionClick: (actionId: string, entry: IActions) => void) => React.ReactNode;
}
...
export { typeTitle, props, parameters };
